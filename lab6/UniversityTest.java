...
class University {
    ...
    public List<String> getAllCourseNames() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.toList())
            .stream().map(l -> l.getName()).collect(Collectors.toList());
    }

    public List<Course> getCoursesWithMinCredits(int minCredits) {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.toList())
            .stream().filter(l -> l.getCredits() >= minCredits).collect(Collectors.toList());
    }

    public int getTotalStudentCount() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.toList())
            .stream().map(l -> l.getEnrolledStudents()).reduce(0, Integer::sum);
    }

    public Optional<Course> getHardestCourse() {

        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.toList())
            .stream().max(Comparator.comparing(Course::getDifficulty));
    }

    public Map<Integer, List<Course>> groupByDifficulty() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.groupingBy(Course::getDifficulty));
    }

    public Map<String, Integer> getCourseEnrollmentMap() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.toMap(Course::getCode, Course::getEnrolledStudents));
    }

    public double getAverageEnrollmentPerCourse() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.averagingDouble(Course::getEnrolledStudents));
    }

    public List<String> getSortedCourseCodes() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).map(l -> l.getCode()).sorted().collect(Collectors.toList());
    }
    ...
    public Map<String, List<String>> getDepartmentToCourseNames() {
        return departments.stream().collect(Collectors.toMap(Department::getName,
                    l -> l.getCourses().stream().map(d -> d.getName()).collect(Collectors.toList())));
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        return departments.stream().max(Comparator.comparingInt(l -> l.getCourses().stream().
                    mapToInt(Course::getEnrolledStudents).sum()));
    }

    public Map<Integer, Integer> getStudentsByDifficulty() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).
    ...
    public List<Course> getCoursesByDifficultyRange(int min, int max) {
        return departments.stream().flatMap(l -> l.getCourses().stream()).
            filter(l -> l.getDifficulty() >= min && l.getDifficulty() <= max).collect(Collectors.toList());
    }

    public List<String> getPopularCourseCodes(int minStudents) {
        return departments.stream().flatMap(l -> l.getCourses().stream()).
            filter(d -> d.getEnrolledStudents() >= minStudents).map(Course::getCode).collect(Collectors.toList());
    }

    public Map<String, Integer> getTotalCreditsPerDepartment() {
        return departments.stream().collect(Collectors.toMap(Department::getName,
                    l -> l.getCourses().stream().mapToInt(Course::getCredits).sum()));
    }

    public List<Course> getTop3HardestCourses() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).
            sorted(Comparator.comparing(Course::getDifficulty).reversed()).limit(3).collect(Collectors.toList());
    }


    public Map<String, Double> getAverageDifficultyPerDepartment() {
        return departments.stream().collect(Collectors.toMap(Department::getName,
                    l -> l.getCourses().stream().collect(Collectors.averagingDouble(Course::getDifficulty))));
    }


    
    public IntSummaryStatistics getEnrollmentStatistics() {
        return departments.stream().flatMap(l -> l.getCourses().stream()).collect(Collectors.summarizingInt(Course::getEnrolledStudents));
    }

    
    public University mergeFourSmallestDepartments() {
        List<Department> sorted = departments.stream().sorted(Comparator.comparingInt(l -> l.getCourses().stream().
                    mapToInt(Course::getEnrolledStudents).sum())).collect(Collectors.toList());

        List<Department> smallest = sorted.stream().limit(4).collect(Collectors.toList());

        Department merged = smallest.stream().reduce((a, b) -> new Department(a.getName() + " & " + b.getName(),
                    Stream.concat(a.getCourses().stream(), b.getCourses().stream()).collect(Collectors.toList()))).orElse(null);

        return new University(Stream.concat(Stream.of(merged), sorted.stream().skip(4)).collect(Collectors.toList()));
    }
}
