...
interface LogProcessor<E extends ILog> {
    ArrayList<E> processLogs(ArrayList<E> logs);
}

class SortByTime<E extends ILog> implements Comparator<E>{
    public int compare(E a, E b){
        return Long.compare(a.getTimestamp(), b.getTimestamp());
    }
}

class LogSystem<E extends ILog>{
    ...
    void printResults() {
        ...
        LogProcessor<E> firstLogProcessor = (logs) -> {
        ...
        LogProcessor<E> secondLogProcessor = (logs) -> {
            ArrayList<E> list = new ArrayList<E>();
            logs.forEach((log) -> {
                if(log.getMessage().length() < 100){
                    list.add(log);
                }
            });
            return list;
        ...
        LogProcessor<E> thirdLogProcessor = (logs) -> {
            logs.sort(new SortByTime<E>());
            return logs;
        };

        System.out.println("RESULTS FROM THE FIRST LOG PROCESSOR");
        firstLogProcessor.processLogs(logs).forEach(l -> System.out.println(l.toString()));

        System.out.println("RESULTS FROM THE SECOND LOG PROCESSOR");
        secondLogProcessor.processLogs(logs).forEach(l -> System.out.println(l.toString()));

        System.out.println("RESULTS FROM THE THIRD LOG PROCESSOR");
        thirdLogProcessor.processLogs(logs).forEach(l -> System.out.println(l.toString()));
    }
}
