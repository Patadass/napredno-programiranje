        ...
        switch(type){
            ...
            case MORE_EXPERIENCE:
                return new Evaluator(){
                    @Override
                    public boolean evaluate(Applicant a){
                        return a.getEmploymentYears() >= 10;
                    }
                };
            ...
            case NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE:
                return new Evaluator(){
                    @Override
                    public boolean evaluate(Applicant a){
                        return !a.hasCriminalRecord() && a.getEmploymentYears() >= 10;
                    }
                };
            case MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE:
                return new Evaluator(){
                    @Override
                    public boolean evaluate(Applicant a){
                        return a.getEmploymentYears() >= 10 && a.getCreditScore() >= 500;
                    }
                };
            case NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE:
                return new Evaluator(){
                    @Override
                    public boolean evaluate(Applicant a){
                        return !a.hasCriminalRecord() && a.getCreditScore() >= 500;
                    }
                };
            default: 
                throw new InvalidEvaluation("Idk");
        }
    }
}

class InvalidEvaluation extends Exception{
    public InvalidEvaluation(String msg){
        super(msg);
    }
}
