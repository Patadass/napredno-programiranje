...
class NumberOfPatientsEvaluator implements DoctorEvaluator{
    public boolean evaluate(Doctor doctor){
        return doctor.getPatients() > 20;
    }
}

class BestDoctorEvaluator implements DoctorEvaluator{
    public boolean evaluate(Doctor doctor){
...
public class HospitalDemo {
    public static void main(String[] args) {
        ...
        Supplier<Doctor> ss = () -> new Doctor(sc.nextInt(), sc.next(), sc.nextInt(), sc.nextInt());
        ...
        Consumer<Doctor> alld = doc -> {
            System.out.println(doc.toString());
        };

        Consumer<Doctor> td = doc -> {
            if(doc.getPatients() > 0){
                System.out.println(doc.toString());
            }
        };
        ...
        Doctor[] passing = er.evaluate(new BestDoctorEvaluator());

        System.out.println("\n=== Doctors with higher number of patients and a higher level of expertise ===");
        for (Doctor d : passing) System.out.println(d);

        Predicate<Doctor> pd = doc -> doc.getLevel() == 10;
        System.out.println("\n=== Chief doctor (level = 10) ===");
        Doctor chief = er.findFirst(pd);
        
        System.out.println(chief != null ? chief : "No chief found");

        Consumer<Doctor> cc = doc -> {doc.setLevel(doc.getLevel() + 1);System.out.println(doc.toString());};
        System.out.println("\n=== Increase all expertise levels by 1 (max 10) ===");
        er.mutate(cc);

        cc = doc -> doc.setLevel(doc.getLevel() + 1);

        Predicate<Doctor> over = (doc) -> doc.getPatients() >= 30;
        System.out.println("\n=== Increase the level of expertise of every doctor by 1 ===");
        er.conditionalMutate(over, cc);

        Function<Doctor, String> mapper = doc -> "Name: " + doc.getName() + ", Level: " + doc.getLevel();
        String[] maped = er.mapToLabels(mapper);
